...
def QUIC(S, lmbda):
    ...
    ro.r('res = QUIC(S,rho = '+str(lmbda)+',msg=0)')
...
def QUIC_lmat(S, lmbdas,Theta0=None):
    ...
    if Theta0 is None:
        ro.r('res = QUIC(S,rho = lmbdas,tol=1e-4,maxIter=100,msg=0)')
    else:
        Theta0 = np.array(Theta0)
        XX = np.array(np.linalg.inv(Theta0))
        tmp2 = ro.r.matrix(XX, nrow = num_vars, ncol = num_vars)
        ro.globalenv['T0'] = tmp2
        ro.r('res = QUIC(S,rho = lmbdas,tol=1e-4,maxIter=100,W.init=T0,msg=0)')
